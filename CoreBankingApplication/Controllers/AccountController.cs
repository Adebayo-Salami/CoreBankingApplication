...
namespace CoreBankingApplication.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //return Content(UserManager.Users.First(u => u.Email == User.Identity.Name).Roles.First().RoleId);
            //if(result == SignInStatus.Success)
            //{
            //    Session["fullname"] = _content.Users.SingleOrDefault(m => m.UserName == model.Email).fullName;
            //    Session["role"] = _content.Users.SingleOrDefault(m => m.UserName == model.Email).role;
            //}
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    Session["fullname"] = _content.Users.SingleOrDefault(m => m.UserName == model.Email).fullName;
                    Session["role"] = _content.Users.SingleOrDefault(m => m.UserName == model.Email).role;
                    Session["tillAcountNumber"] = _content.Users.SingleOrDefault(m => m.UserName == model.Email).tillAccount;
                    if(Session["role"].ToString() == "Teller")
                    {
                        Session["tillBal"] = _content.tellerDetails.SingleOrDefault(m => m.tellerUsername == model.Email).tillBalance;
                    }
        ...
        public async Task<ActionResult> Register(AccIndexer model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.reg.Email, Email = model.reg.Email,  Branch = model.reg.Branch, tillAccount = till, PhoneNumber = model.reg.PhoneNumber, homeAddress = model.reg.HouseAddress, fullName = model.reg.FullName, role = model.reg.role};
